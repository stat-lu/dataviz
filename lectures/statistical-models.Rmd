...
For analysis, we often also want **diagnostic**, **estimates**, and **prediction** plots.
...
For lm() models, usually only the standard plot() method is needed.

.pull-left[
#### Residual Plot

- a useful standard diagnostic plot
...
It is easy to pipe the `broom::tidy()` output into ggplot2 in order to create coefficient plots.
...
<ol>
<li> create a table of values to predict for
</ol>
...
<ol start=2>
<li> augment new data with predictions from the model using <code>broom::augment()</code>
</ol>
...
```{r, fig.width = 10, fig.height = 5, fig.cap = "Prediction heatmaps for fertility across variables education, infant mortality, and religion."}

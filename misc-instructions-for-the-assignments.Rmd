...
If you prefer to use a WYSIWYG editor, use
`ggplot2::ggsave()` to save your plots to your hard drive and then input them
manually into your document. Here is a simple example of how to do this:
...
You need to submit a PDF with your work. If you'd like to, however, you are also
free to include `.Rmd` and `.R` files with your submissions containing code to
reproduce your work. Doing so, however, is not at all obligatory.
...
included. Use a walk-through approach, where you guide the reader through the
report. Someone who has not seen the assignment instructions but with the same
background as you should be able to read and follow along reasonably well. Do
not use a question: answer format in your report.

### Figures

Any figures you include should have descriptive labels and captions. If you are
using R Markdown, please use the `fig.cap` argument in the R Markdown chunk
header to provide a caption. Also make use of `fig.width` and `fig.height` to
control the size of the figures. A code chunk for a figure might look something
like this:
...
If you use a WYSIWYG editor, you need to first import your images into the
editor and then right-click on them and choose `Insert Caption` (or some similar
command). **Don't use `ggtitle()`, `labs(title = ...)`!**

See [The Anatomy of a
Plot](https://stat-lu.github.io/dataviz/misc-the-anatomy-of-a-plot) for examples
and more instruction on how to format your figures.

### Code

Only include code when you are **instructed** to do so. When you do include
code, please avoid having it escape into the margin by using multiple lines to
separate code. Good code style is not a requirement in this course but is
nevertheless good practice. It might be a good idea to conform to a style guide
such as [the tidyverse style guide](https://style.tidyverse.org/). R Studio can
also format code for you if you select it and hit `Cmd/Ctrl + Alt + a`.

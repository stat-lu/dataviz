...
So how does this work? The key to this is that `geom_bar()` computes (and makes
accessible) certain statistics. Among these are `prop`, which is the
*group-wise* proportion. To access this statistic, we need to use the special
`after_stat()` function, which acts as a wrapper that evaluates its arguments
after the statistics in the enclosing function have been computed. To get this
to work properly, we also need to map the `group` aesthetic, so that ggplot
knows which category to compute proportions within.

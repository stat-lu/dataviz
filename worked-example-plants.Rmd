...
knitr::include_graphics(paste0(
  ...
  "Echinochloa_crus-galli01.jpg/220px-Echinochloa_crus-galli01.jpg"
))
```

### Data

The data we are going to look at is available in the `CO2` data set from
**data sets** (a package that is part of the base R distribution). There are five
variables:
...
concentration. We'll of course map uptake to the y axis, and concentration to
the x axis. Before we get started, we'll also rename some of the variables so
that we get decent axis labels by default and change `Plant` from ordered to
unordered (since the variable obviously isn't ordinal). We'll call the new data
set `co2`.
